 ...
 * AI 가 장소 · 한 일 · 특이사항 칸으로 나눠 넣는다. 이미 채운 뒤 더 받아쓰고 다시 누르면 덧붙여 정리한다.
...
export default function EntryForm({ initial, submitLabel, onSubmit, onCancel, onDirtyChange }: Props) {
  ...
  const organize = async () => {
    ...
    try {
      ...
      setNote(mode === 'new'
        ? 'AI가 「한 일」의 글을 장소 · 한 일 · 특이사항으로 나눠 넣었습니다. 확인하고 저장해 주세요.'
  ...
  const help = aiConnected
    ? filled
      ...
      : `「한 일」 칸에 ${micWord}로 오늘 간 곳 · 한 일 · 특이사항을 받아쓴 뒤 「AI 로 정리」를 누르면 칸별로 나눠 줍니다.`
    : `「한 일」 칸에 ${micWord}로 받아쓰거나 직접 적어 주세요. (설정에서 AI 를 연결하면 받아쓴 글을 장소 · 한 일 · 특이사항 칸으로 나눠 줍니다)`
  ...
  return (
    <form className="form" onSubmit={submit} onKeyDown={onKeyDown}>
      ...
      <div className="field">
        <label htmlFor="f-client">장소 <span className="opt">선택</span></label>
        <input id="f-client" type="text" placeholder="예) 문고, 중앙도서관" maxLength={60} {...bind('client_name')} />
      </div>
      ...
      <div className="field care-field">
        ...
        {aiConnected && (
          <div className="organize-row">
            ...
            <span className="help">
              {organizing ? '보통 5~15초 걸립니다. 잠시만 기다려 주세요.' : '장소 · 한 일 · 특이사항으로 나눠 넣습니다'}
            </span>
          </div>
        )}
      </div>
